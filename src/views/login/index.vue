  ...
  <div class="login-container">
    <el-form ref="loginForm" :model="loginForm" :rules="loginRules" class="login-form" auto-complete="on" label-position="left">
      ...
      <el-form-item prop="mobile">
        ...
        <!-- tabindex="1"属性：摁下了 tab 键，光标会自动转到下一个输入框 -->
        <!-- auto-complete="on"属性：自动填充输入框内容的操作，on 表示开启；off 表示关闭 -->
        <el-input
          ref="username"
...
export default {
  name: 'Login',
  data() {
    // 事先定义手机号校验规则，然后添加到 validator 自定义函数上
    const validateMobile = (rule, value, callback) => {
      // callback 是一个回调函数，不论验证成功还是失败，这个 callback 都会执行
      // 校验成功 callback（）
  ...
  methods: {
    ...
    async  handleLogin() {
      ...
      try {
        // btn 按钮动画
        this.loading = true
        // 先验证表单,validate 是 element ui 组件自带的校验方法，如果不给参数，则返回一个 promise 对象
...
.login-container {
// 在 css 样式里面也可以书写 @ 别名，但是前面的 ~ 符号一定要加
