      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            ...
            <ImageUpload1 ref="newStaffPhoto" />
          </el-form-item>
        </el-col>
      </el-row>
      <!-- 保存个人信息 -->
    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
          <!-- <ImageUpload ref="staffPhoto2" /> -->
          <ImageUpload1 ref="newStaffPhoto1" />
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async getUserDetailById() {
      ...
      // ------------------
      if (this.userInfo.staffPhoto && this.userInfo.staffPhoto.trim()) {
        this.$refs.newStaffPhoto.fileList = [{ url: this.userInfo.staffPhoto, upload: true }]
      }
    },
    // 获取员工详细信息
    async getPersonalDetail() {
      ...
      //   this.$refs.staffPhoto2.fileList = [{ url: this.formData.staffPhoto, upload: true }]
      // }
      // --------------
      if (this.formData.staffPhoto && this.formData.staffPhoto.trim()) {
        this.$refs.newStaffPhoto1.fileList = [{ url: this.formData.staffPhoto, upload: true }]
      }
    },
    // 保存员工基本信息
    async saveUser() {
      // 先获取头像地址
      const fileList = this.$refs.newStaffPhoto.fileList
      // 判断当前的图片有没有上传完成
      // 看看有没有图片的状态取反之后为 true 的，有的话代表上传未完成
      // if (fileList.some(item => !item.upload)) {
      //   this.$message.warning('图片还未上传完成')
      //   return
      // }
      // -----------
      const file = this.$refs.newStaffPhoto.fileList.length
      if (!file) {
        return this.$message.warning('请上传')
      }
      // 开始给 userInfo 添加头像属性 ↓
      // this.userInfo.staffPhoto = fileList[0].url
      // 调用接口时重新修改 staffPhoto 属性；staffPhoto 由于接口问题，必须给一个有空格的字符串才能存进去
    ...
    // 保存员工详细信息
    async savePersonal() {
      const fileList = this.$refs.newStaffPhoto1.fileList
      // if (fileList.some(item => !item.upload)) {
      //   this.$message.warning('图片还未上传完成')
      //   return
      // }
      const file = this.$refs.newStaffPhoto1.fileList.length
      if (!file) {
        return this.$message.warning('上传未完成')
      }
      await updatePersonal({ ...this.formData, staffPhoto: fileList.length ? fileList[0].url : ' ' })
      console.log(this.formData.staffPhoto)
      this.$message.success('保存员工详细信息成功')
      this.getPersonalDetail()
    }
  }
}
